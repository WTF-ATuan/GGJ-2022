		...
		public void OnTriggerEnter(Collider other){
			...
			if(enemy != null){
				...
				if(canBeatenActor){
					...
					// 產生反擊子彈
					Enemy_Backtrack enemy_Backtrack = Instantiate(AttackMagnet._.enemy_Backtrack,
					...
					enemy_Backtrack.End_Act += () => {
						if(enemy.magneticPole == MagneticPole.North){
							SoundManager.instance.PlaySoundEffect("被扁N");
							AttackMagnet._.HP_N--;
						}
						else{
							SoundManager.instance.PlaySoundEffect("被扁S");
							AttackMagnet._.HP_S--;
						}
